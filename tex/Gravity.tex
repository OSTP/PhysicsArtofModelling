...
Here's an analogy that may help you understand Gauss's Law conceptually:

A famous celebrity is doing an event, and they attract a certain number of fans. The fans will keep walking towards the celebrity unless you put up a barricade around the celebrity. If another celebrity shows up at the same event, they will attract their own fans, so there will be twice as many people around the barricade. With Gauss's law, the strength of the gravitational field is like how crowded it feels for someone standing at the barricade. The number of celebrities is kind of like the enclosed mass $M^{enc}$; the more mass that is enclosed, the stronger the field will be. 

Now imagine all of the fans have just arrived and you haven't put up a barricade yet. A photographer is coming to the event, and you told them to stand at some location that is a distance $r$ from the celebrities so that he has the best view. The photographer wants to know how crowded he will feel when he is standing behind the barricade at that location. You want to give him an answer quickly, so you want to know the easiest way to figure it out. 
Gauss's law is like saying ``If the people are evenly spread out around the barricade, and I know how many celebrities are at the event and how long my barricade is, I can figure out how crowded it feel for the photographer."

The barricade is like our Gaussian surface, and like a Gaussian surface, we can put it wherever we want in whatever shape we want. So, we can build it in such a way that the fans will be evenly spread out.

This is an analogy for the gravitational field, so there's something else we know: the gravitational field is the same so long as the distance to the centre of the mass is the same. In our analogy, this is like saying that the fans have to be evenly spread out when they are all the same distance $r$ from the celebrity. This means that we have to build a circular barrier, so that we can easily figure out how crowded the photographer will feel. 
\end{studentOpinion}
