...
\begin{opening}{A rocketship follows a circular orbit of radius $r$ around Earth. The rocket quickly accelerates to twice its initial speed (so it is essentially instantaneous).}

\begin{MCquestion}{Assuming the rocket remains in orbit, what will be the shape of its new orbit?}
\item A circle of radius $r$.
\item A circle with a radius not equal to $r$.
\item An ellipse centred around the Earth.
\item An ellipse not centred around the Earth. \correct
\end{MCquestion}
\begin{MCquestion}{What will happen to the period of the rocket's orbit?}
\item It will be the same as its initial period.
\item It will be greater than its initial period. \correct
\item It will be less than its initial period. 
\end{MCquestion}
\end{opening}
...
\begin{checkpoint}
\begin{MCquestion}{An object in a circular orbit, radius $r$, has an orbital speed $v$. 

If you double the radius of the circular orbit, what will be the value of the orbital speed?}
\item $2v$
\item $8v$
\item $\sqrt{8}$.
\item $\frac{1}{\sqrt{8}}$.
\item $\frac{2}{\sqrt{8}}$. \correct
\end{MCquestion}
\end{checkpoint}
TODO: Check answer
...
\begin{checkpoint}
\begin{MCquestion}{There are two planets located a distance $d$ apart. Point A is located midway between the two planets. Point B is located a distance $d/2$ from one of the planets, at the location shown in Figure \ref{fig:gravity:fieldAB}. How does the field at A compare to the field at B?\capfig{0.7\textwidth}{figures/Gravity/fieldAB.png}{\label{fig:gravity:fieldAB}Two planets are a distance $d$ apart. We are considering the gravitational field at two points, A and B, located near the planets.}}
