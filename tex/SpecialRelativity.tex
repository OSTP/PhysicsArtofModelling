...
\end{align*}
where $(x_A,y_A,z_A,ct_A)$ and $(x_B,y_B,z_B,ct_B)$ are the space-time coordinates of events $A$ and $B$. The above equation must hold in all frames of references (e.g. adding a prime to each coordinate), since it is a statement that the speed of light is $c$.
...
Einstein's relation is often used to express the mass of subatomic particles in terms of energy. For example, an electron has a mass of $\SI{511e3}{eV/c^2}$ in these units. The electron-volt, $eV$, is the amount of kinetic energy that an electron would have after being accelerated through $\SI{1}{V}$, and corresponds to $\SI{1}{eV}=\SI{1.6e-19}{J}$.
...
The Lorentz transformations allow us to convert the coordinates of events in one frame of reference, $S$, to those in a frame, $S'$, moving with constant speed, $v$, relative to $S$:
\begin{align*}
x' &=\gamma (x-vt)\\
y' &= y\\
z' &= z\\
...
s^2 = \Delta x^2 + \Delta y^2 +\Delta z^2&-c^2\Delta t^2
\end{align*}
One can think of this as a sort of ``distance'' in space-time, which does not depend on the frame of reference.
...
In order for momentum and energy to be conserved in Special Relativity, these need to be redefined. If a particle with rest mass, $m_0$, has a velocity, $\vec u$, in an inertial frame of reference, its relativistic momentum, $\vec p$, is defined to be:
\begin{align*}
...
\begin{chapteractivity}{To try in the lab}
	{
	 ...
	 \item Propose an experiment to test relativistic effects with electromagnetism.
	}
\end{chapteractivity}
