...
\end{MCquestion}}
\end{checkpoint}
\begin{example}{A monkey is hanging from a tree branch. You want to feed the monkey by throwing it a banana. You know that the monkey is easily frightened and will let go of the tree branch the instant you throw the banana. The monkey is a horizontal distance $d$ away from you and a height $h$ above you. What angle should you throw the banana at so that you hit the monkey?}
...
First, we need to set up our coordinate system. We set the origin to be where you are standing. This is the initial position of the banana. We let $y$ be in the vertical direction and let $x$ be in the horizontal direction. We let ``up" be positive and let the values of $x$ increase as you get closer to the tree, as shown in Figure \ref{fig:MonkeyTreeExample}. \\

We treat the components of the banana's and the monkey's motion as being independent. The monkey's motion has only a vertical component. The acceleration of the monkey in the vertical direction is the acceleration due to gravity, $g$, which points in the $-y$ direction. The initial vertical position of the monkey is $h$ and its initial vertical velocity is 0, so we can describe the vertical position of the monkey, $y_M$, by:
\begin{align*}
y_M&=h-\frac{1}{2}gt^2
\end{align*}
The horizontal position of the monkey is constant, and is equal to $d$.\\
...
x_B&=v_{xB}t
\end{align*}
We defined the initial vertical position of the banana to be $y_{0B}=0$. The vertical position, $y_B$, can thus be described by:
\begin{align*}
y_B&=v_{0yB}t-\frac{1}{2}gt^2
\end{align*}
Now that we have our equations, we can use our conditions for a successful hit. For the monkey and the banana to be in the same position, we need $y_M=y_B$ and $x_B=d$. Setting our equations for $y_M$ and $y_B$ equal to one another gives:
\begin{align*}
h-\frac{1}{2} gt^2&=v_{0yB}t-\frac{1}{2}gt^2\\
h&=v_{0yB}t
\end{align*}
And setting $x_B$ to be equal to $d$ gives:
\begin{align*}
d&=v_{xB}t
\end{align*} 
Because these conditions have to be true at the same time for us to hit the monkey, we can just divide our equations to find:
\begin{align*}
...
\capfig{0.7\textwidth}{figures/DescribingMotionInND/MonkeyTreeExample.png}{\label{fig:MonkeyTreeExample} Feeding a monkey in a tree.}
\end{example} 
