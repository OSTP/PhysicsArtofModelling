...
\begin{definitions}
	...
	\textbf{Angle:}
	\item The angular distance between two points as viewed by an external observer at an arbitrary distance from the two points. SI units: none. Common variable(s): $\theta$, $\phi$.
	\medskip
	\item \textbf{Angular velocity:}
	\item The rate at which an angle changes with respect to time. SI units: \SI{}{s^{-1}}. Common variable(s): $\omega$.
	\medskip
	\item \textbf{Angular acceleration:}
	\item The rate at which angular velocity changes with respect to time. SI units: \SI{}{s^{-2}}. Common variable(s): $\alpha$.
\end{definitions}
