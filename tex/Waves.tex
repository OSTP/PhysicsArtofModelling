...
\item Write the incident pulse, the reflected pulse, and the transmitted pulse in the form $D(x,t)=f(t\pm x/v)$, where $f$ is some function that depends on either $t+x/v$ or $t-x/v$, depending on the direction that the pulse is travelling in.
...
\end{align*}
Using the fact that $k=2\pi/\lambda$ and $\omega=2\pi f$, this equation can be rewritten to take the form $D=f(t- x/v)$:
\begin{align*}
...
\end{align*}
where we noted that $\cos(-x)=\cos(x)$.
...
A_i+A_R&=A_T
\end{align*}
Unless there is a kink in the rope, the slope of the rope must also be continuous at the boundary. Since the incident and reflected pulses superimpose, and the principle of superposition states that the net displacement is simply the sum of the displacement of these two waves, we can write:
\begin{align*}
...
\end{align*}
Using our earlier condition, $A_i+A_R=A_T$, we get:
\begin{align*}
