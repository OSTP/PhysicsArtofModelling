...
\capfig{0.8\textwidth}{figures/Waves/sinewaveaxes.png}{\label{fig:waves:sinewaveaxes}The displacement ($D$) of points at different positions ($x$) on a rope as a sine wave passes through.}
...
\begin{example}{Show that the function $D(x,t) = A\sin\left( kx -\omega t + \phi \right)$ satisfies the one-dimensional wave equation.}The wave equation is the condition:
\begin{align*}
\die{^2D}{x^2}=\frac{1}{v^2}\die{^2D}{t^2}
\end{align*}
...
\capfig{0.7\textwidth}{figures/Waves/sphericalwave.png}{\label{fig:waves:sphericalwave}Left: Before you start clapping, the air molecules are evenly distributed. We divide the air up into thin spherical shells. They are in their equilibrium position and all have the same density. Right: When you start clapping, these shells alternately expand and contract about their equilibrium positions.}
...
Here's one way to remember that the amplitude decreases as a function of distance from the source: The further a shell is from the centre, the more particles it contains (since the density of the medium is constant). When the energy is transferred from one shell to the next, the total energy stays the same. Since the energy must be split between a greater number of particles, each particle gets less energy, and therefore oscillates with a smaller amplitude than the particles in the previous shell did.
\end{studentOpinion}
...
\end{align*}
This looks like the equation for a stationary wave (the displacement is a function of $x$) with an amplitude $2A\cos(\omega t)$. We know that $\cos(\omega t)$ will give a value that ranges between -1 and 1, so we can just think of it as a scaling term that modifies the amplitude of the wave. 
...
\subsection{Problems}
\begin{problemParts}{soln:waves:reflectioncoeff}{\label{prob:waves:reflectioncoeff}A pulse propagates down a rope of mass per unit length $\mu_1$ that is tied to a second rope with a mass per unit length $\mu_2$ (the tensions in the ropes are equal in magnitude).}
\item Write the incident pulse, the reflected pulse, and the transmitted pulse in the form $D=f(t\pm x/v)$ where $f$ is a function that will depend on either $t+x/v$ or $t-x/v$ depending on the direction that the pulse is travelling in.
\item Using the boundary conditions, show that the reflection coefficient, $R$ , (the ratio of the amplitude of the reflected pulse to the amplitude of the incident pulse) is given by:
\begin{align*}
R=\frac{\sqrt{\mu_1}-\sqrt{\mu_2}}{\sqrt{\mu_1}+\sqrt{\mu_2}}
\end{align*}
\end{problemParts}

\begin{problemParts}{soln:waves:clarinet}{\label{prob:waves:clarinet} A clarinet can be modelled as an air column that is open at one end and closed at the other end.}
...
\begin{enumerate}[(a)] 
\item We let the initial pulse move in the positive $x$ direction (Figure \ref{fig:waves:reflectioncoeff}).
\capfig{0.5\textwidth}{figures/Waves/reflectioncoeff.png}{\label{fig:waves:reflectioncoeff} The wave propagates through a rope connected to a another rope with a different linear mass density. Whether the reflected pulse is inverted or upright will depend on the reflection coefficient.}

 The incident pulse can then be described by the function:
\begin{align*}
D_i=A_i\cos(k_1x-\omega t)\\
\end{align*}
This can be rewritten to take the form $D=f(t- x/v)$:
\begin{align*}
D_i&=A_i\cos\left( \frac{2\pi}{\lambda_1}x-2\pi ft\right)\\
&=A_i\cos \left( 2\pi(\frac{1}{\lambda_1}x- ft)\right)\\
&=A_i\cos \left( 2\pi f(\frac{x}{v_1}- t)\right)\\
&=A_i\cos \left( -2\pi f(t-\frac{x}{v_1})\right)\\
D_i&=A_i\cos \left(2\pi f(t-\frac{x}{v_1})\right)
\end{align*}
The transmitted wave will also travel in the positive $x$ direction but at a different speed (because the speed depends on the mass density of the rope):
\begin{align*}
D_T&=A_T\cos \left( 2\pi f(t-\frac{x}{v_2})\right)
\end{align*}
The reflected wave will travel in the $-x$ direction and at the same speed as the incident pulse.
\begin{align*}
D_R&=A_R\cos \left( 2\pi f(t+\frac{x}{v_1})\right)
\end{align*}
\item We will consider the boundary conditions at the interface between the two ropes (we set this location on the rope to be $x=0$). One boundary condition is that the the rope must be continuous. As a result, the vertical displacement on the $-x$ side of the boundary must be the same as the vertical displacement on the $+x$ of the boundary. Since the front of the pulse will reach the boundary before the end of the pulse, the incident and reflected pulses will superpose. This can be expressed as:
\begin{align*}
A_{-x}&=A_{+x}\\
A_i+A_R&=A_T
\end{align*}
Another condition is that, at the boundary, the net force in the $y$ direction must be zero, or else the ropes would disconnect:
\begin{align*}
F_T\sin\theta_1=F_T\sin\theta_2\\
\sin\theta_1=\sin\theta_2
\end{align*}
where we noted that the magnitude of the tension is the same in both ropes. The angles are shown in Figure \ref{fig:waves:reflectioncoeffangle}
\capfig{0.5\textwidth}{figures/Waves/reflectioncoeffangle.png}{\label{fig:waves:reflectioncoeffangle} The forces acting on the rope at the boundary.}

For small angles, $\sin\theta \approx \tan\theta$. Since $\tan\theta$ is the slope of the rope, we can write:
\begin{align*}
...
\end{align*}
Using our earlier condition, $A_i+A_R=A_T$, and simplifying:
\begin{align*}
\frac{A_i}{v_1} -\frac{A_R}{v_1}&=\frac{A_i}{v_2}+\frac{A_R}{v_2}\\
\end{align*}
Now we can find the reflection coefficient:
\begin{align*}
A_i\left( \frac{v_2-v_1}{v_1v_2}\right)&=A_R\left( \frac{v_2+v_1}{v_1v_2}\right)\\
R&=\frac{v_2-v_1}{v_2+v_1}
\end{align*}
Since the velocities in the first and second rope are $v_1=\sqrt{F_T/\mu_1}$ and $v_2=\sqrt{F_T/\mu_2}$, the reflection coefficient can be stated as:
\begin{align*}
R=\frac{\sqrt{\mu_1}-\sqrt{\mu_2}}{\sqrt{\mu_1}+\sqrt{\mu_2}}
\end{align*}
\end{enumerate}
...
\begin{enumerate}[(a)]
\item \capfig{0.5\textwidth}{figures/Waves/clarinet.png}{\label{fig:waves:clarinet} The first three harmonics for a clarinet. There is a node at the fixed end and an anti-node at the free end.}
...
\end{align*}
where we used the fact that $k=2\pi /\lambda$. We can check that this formula works for the first three harmonics:
\begin{align*}
...
\lambda=\frac{4L}{2n-1}
\end{align*}
Writing $\lambda$ in terms of the velocity, $v$, and frequency, $f$, and rearranging gives:
\begin{align*}
\frac{v}{f}&=\frac{4L}{2n-1}\\
f&=\frac{v(2n-1)}{4L}
\end{align*}
If we want to find the lowest frequency (and therefore the largest wavelength), we want $n=1$. The speed of sound in air  at room temperature is $\SI{343}{m/s}$ and the length of the clarinet is $\SI{0.6}{m}$. Now we can solve for $f$.
\begin{align*}
f&=\frac{(\SI{343}{m/s})(2(1)-1)}{4(\SI{0.6}{m})}\\
f&=\SI{143}{Hz}
\end{align*}
Discussion: This is close to the $D_3$ note, which has a frequency of $\SI{144}{Hz}$, so this answer makes sense. Why might our answer differ from the true value?
\end{enumerate}
\end{solution}
