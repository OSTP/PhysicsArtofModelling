...
\begin{definitions}
\textbf{Wavelength:} The distance between two successive maxima ("peaks") or minima (troughs) in a wave. SI units: [\SI{}{m}]. Common variable(s): $\lambda$.
\medskip
\item \textbf{Amplitude:} The maximal distance that a particle in a medium is displaced from its equilibrium position. SI units: [\SI{}{m}]. Common variable(s): $A$.
\medskip
\item \textbf{Frequency:} The number of cycles of oscillatory motion that occur in one second. SI units: [\SI{}{s^{-1}}]. Common variable(s): $f$.
\medskip
...
\medskip
\item \textbf{Volume mass density:} The mass per unit volume of an object.. SI units: [\SI{}{kgm^{-3}}]. Common variable(s): $\rho$.
\medskip
\item \textbf{Intensity:} The power per unit area transmitted by a wave. SI units: [\SI{}{Wm^{-2}}]. Common variable(s): $I$.
\medskip
\end{definitions}
\newpage
