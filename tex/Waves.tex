...
When you look at the ripples (waves) made by a rock dropped in a pond (Figure \ref{fig:waves:water}), those ripples travel away from where the rock entered the water, but there is no motion of water outwards. Individual water molecules will move in little circles about an equilibrium position, but they do not move along with the waves.
...
If you clap your hands, you will create a pressure disturbance in the air that will propagate; this is what we call sound (a sound wave). Again, it is not air molecules that move, it is the disturbance that moves through the air. 
...
At time $t=0$ (solid line), we the displacement of each point in the medium, $D(x, t=0)$, as a function of their distance from the origin, $x$, can be described by a sine function:
\begin{align*}
D(x,t=0) = A\sin\left( \frac{2\pi}{\lambda}x \right)
\end{align*}
This corresponds to the displacement being 0 at the origin and at any and position, $x$, which is a multiple of the wavelength, $\lambda$. If the wave moves with velocity $v$ in the positive $x$ direction, then, at time, $t$, all the sine function in Figure \ref{fig:waves:sinewave} will have shifted to the right by an amount $vt$. The displacement of a point located at position $x$ at time $t$, will be the same as the displacement of the point at position $x-vt$ at time $t=0$.  We can state this condition as:
\begin{align*}
D(x,t) = D(x-vt, t=0)
\end{align*} 
That is, at some time $t$, the displacement of a point at position $x$ is found by subtracting $vt$ from $x$ and taking the displacement of the point at $x-vt$ at $t=0$. We can write a function for the displacement of a point at position $x$ and a time $t$, as:
\begin{align*}
...
v=\sqrt{\frac{E}{\rho}}
\end{align*}
where $E$ is the ``elastic (or Young's) modulus'' for the material, and $\rho$ is the density of the material. The elastic modulus of a solid is a measure of how much the solid deforms when a force (or pressure) is exerted on the material, and the density is a measure of the mass of the particles that make up the solid.
...
\begin{checkpoint}
\begin{MCquestion}{Will a wave propagate faster through}
\item ice.\correct
\item water.
\end{MCquestion}
\end{checkpoint}
...
Consider a shell at a radial position, $r$, from the source, with thickness $dr$, and mass $dm$. If the medium has a density, $\rho$, then the mass of the shell is given by:
\begin{align*}
