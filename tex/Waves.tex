...
TODO: thoughtbox on an alternate derivation.
...
Finally, consider a pulse that propagates down a rope of mass per unit length $\mu_1$ that is tied to a second rope with mass per unit length $\mu_2$. When the pulse arrives at the interface between the two media (the two ropes), part of the pulse will be reflected back, and part will be transmitted into the second medium (Figure \ref{fig:waves:transmission}). 
...
By considering the boundary conditions, one can derive the coefficient of reflection, $R$. This coefficient is the ratio of the amplitude of the reflected pulse to the amplitude of initial pulse. The ratio is found to be:
\begin{align*}
...
\capfig{0.7\textwidth}{figures/Waves/sphericalwave.png}{\label{fig:waves:sphericalwave}Left: Before you start clapping, the air molecules are evenly distributed. We divide the air up into thin spherical shells. They are in their equilibrium position and all have the same density. Right: When you start clapping, these shells alternately expand and contract about their equilibrium positions. The mass of each shell stays the same.}
...
TODO: What about when one end/neither end is fixed. Maybe a checkpoint question?
\newpage
\section{Summary}
