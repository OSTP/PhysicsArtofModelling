...
\begin{example}{You push horizontally with a force $\vec F$ on a box of mass $m$ that is resting against a vertical wall, as shown in Figure \ref. The coefficient of static friction between the wall and the box is $\mu_s$. What is the minimum magnitude of the force that you must exert for the ball to remain stationary?}
...
Since the box does not move, we can build a model where the sum of the forces exerted on the box is zero. We start by identifying the forces exerted on the box; these are:
\begin{enumerate}
\item $\vec F$, the horizontal force you apply on the box.
...
\end{enumerate}
The forces are shown in the free-body diagram in Figure \ref{fig:applyingnewtonslaws:blockwall_fbd}, along with our choice of coordinate system.
\capfig{0.2\textwidth}{figures/ApplyingNewtonsLaws/blockwall_fbd.png}{\label{fig:applyingnewtonslaws:blockwall_fbd} Free-body diagram of the forces applied on the box.}
We can then write the $x$ component of Newton's Second Law:
\begin{align*}
\sum F_x = F - N &=0\\
\therefore N = F
\end{align*}
which tells us that the normal force exerted by the wall as the same magnitude as the applied force, $\vec F$. The $y$ component of Newton's Second Law is:
\begin{align*}
\sum F_y f_s - F_g &=0\\
\therefore \mu_s N -mg &=0\\
\end{align*}
Note that we set the force of friction to have a magnitude $f_s = \mu_s N$. The question asks for the minimum force with which you must push on the box. From the $x$ component of Newton's Second Law, we found that the normal force is equal to the applied force. If you push with a smaller force, then the normal force will be smaller. The maximal value of the force of static friction is $f_s = \mu_s N$, and so the force of friction is also proportional to the force that you exert (since $N$ is proportional to the force that you exert). RESUME HERE!
\end{example}
