...
\begin{checkpoint}
\begin{MCquestion}{With reference to the block in figure \label{fig:applyingnewtonslaws:circleH}, what path will the block take if the string breaks?}
\item \capfig{0.25\textwidth}{figures/ApplyingNewtonsLaws/trajectoryA.png}{\label{fig:applyingnewtonslaws:trajectoryA}}
\item \capfig{0.25\textwidth}{figures/ApplyingNewtonsLaws/trajectoryB.png}{\label{fig:applyingnewtonslaws:trajectoryB}}
\item \capfig{0.38\textwidth}{figures/ApplyingNewtonsLaws/trajectoryC.png}{\label{fig:applyingnewtonslaws:trajectoryC}}
\item \capfig{0.25\textwidth}{figures/ApplyingNewtonsLaws/trajectoryD.png}{\label{fig:applyingnewtonslaws:trajectoryD}}
\end{MCquestion}
\end{checkpoint}
...
\textbf{Goldilocks and the 3 banked curves}

Figuring out which way the force of static friction points when you are travelling around a banked curve can be helpful with following analogy:

The force of static friction depends on the speed of your car, rather than anything else, so it can be helpful to think of the force of static friction as a sort of ``Goldilocks''. The goal of Goldilocks is to get a porridge that is ``just right'', rather than having a porridge that is too lumpy or runny - she likes a balanced porridge. The force of static friction has a similar goal; it wants the forces on the car to be balanced ``just right'' so that the car doesn't slide down or slip off of the curve. 

If the porridge already happens to be ``just right'' (not too lumpy, not too runny), Goldilocks won't have to make any changes before eating the porridge. Similarly, if your speed happens to be ``just right'', the x component of the normal force is enough to provide the perfect acceleration for the car to continue on a straight path. The force of static friction won't make any changes, and will be zero.

If the porridge is too runny, Goldilocks will have to add something to thicken it to make it ``just right''. Analogously, if the car is too slow and the car starts sliding downwards, the force of static friction will act upwards to help the car to travel in a straight line, ``just right''. 

If the porridge is too lumpy, Goldilocks will have to add water to make it ``just right''. If the car is too fast and is about to slip out of the curve, the force of static friction will act downwards to make the car travel in a straight line, ``just right''. 

In the same way that Goldilocks acts differently whether her porridge is runny or too lumpy, the force of static friction will act differently whether the car is travelling too fast or too slow.
...
\begin{problemParts}{soln:applyingnewtonslaws:pendulum}{\label{prob:applyingnewtonslaws:pendulum} Consider a conical pendulum with a mass m, attached to a string of length L, such as the one depicted in figure \ref{fig:applyingnewtonslaws:conicalpendulum}. If the pendulum travels through a circular path of radius r and the string makes an angle theta with the vertical, derive}
...
\capfig{0.4\textwidth}{figures/ApplyingNewtonsLaws/rollercoaster.png}{\label{fig:applyingnewtonslaws:rollercoaster} The roller coaster}
\end{problem}
...
\capfig{0.25\textwidth}{figures/ApplyingNewtonsLaws/conicalpendulumfbd.png}{\label{fig:applyingnewtonslaws:conicalpendulumfbd} Forces acting on the conical pendulum}

Figure \ref{fig:applyingnewtonslaws:conicalpendulumfbd} this figure shows a free-body diagram for the forces on the mass along the circular path of the pendulum. The pendulum is not accelerating vertically. Writing the y component of Newton's Second law gives:
\begin{align*}
\sum F_y=0 \\
Tcos\theta -F_g=0 \\
Tcos\theta=mg \\
...
The forces in the frame of reference of Barb and Kenny are illustrated below:

\capfig{0.15\textwidth}{figures/ApplyingNewtonsLaws/rollercoasterfbd.png}{\label{fig:applyingnewtonslaws:rollercoaster} The forces acting on the roller coaster.}
