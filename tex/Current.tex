...
In this chapter, we introduce tools to model electric current, namely, the motion of charges inside a conductor. We will show how we can connect the microscopic motion of electrons to macroscopic quantities, such as current and voltage, which can be measured in the laboratory. We will also introduce the notion of resistance, as well as the resistor, a common component in electric circuits.
...
Understanding the Fermi velocity requires quantum mechanics and is beyond the scope of this textbook. However, the Fermi velocity is representative of the actual velocity of electrons in a conducting material, mostly due to their thermal energy. In a good conductor, these speeds are roughly 1/200 the speed of light.
...
\hline
\textbf{Material} & 
\textbf{Resistivity} [\si{\Omega\cdot m}] & 
\parbox[t]{3.516cm}{\textbf{Temperature coefficient} [\si{\degree C^{-1}}]} & 
\parbox[t]{3.516cm}{\textbf{Free electron density} [\si{m^{-3}}]} \\
\hline
Silver & \num{1.59e-8} & \parbox[t]{3.516cm}{0.0038} & \parbox[t]{3.516cm}{\num{5.86e28}} \\
Copper & \num{1.68e-8} & \parbox[t]{3.516cm}{0.0040} & \parbox[t]{3.516cm}{\num{8.46e28}} \\
Gold & \num{2.44e-8} & \parbox[t]{3.516cm}{0.0034} & \parbox[t]{3.516cm}{\num{5.90e28}} \\
Aluminum & \num{2.74e-8} & \parbox[t]{3.516cm}{0.0039} & \parbox[t]{3.516cm}{\num{18.1e28}} \\
Iron & \num{9.70e-8} & \parbox[t]{3.516cm}{0.0050} & \parbox[t]{3.516cm}{\num{17.0e28}} \\
Silicon & 0.1–1000 & \parbox[t]{3.516cm}{-0.0750} & \parbox[t]{3.516cm}{\num{0}} \\
Rubber & $(1\text{-}100)\times 10^{13}$ & \parbox[t]{3.516cm}{0} & \parbox[t]{3.516cm}{\num{0}} \\
Quartz & $7.5\times 10^{17}$ & \parbox[t]{3.516cm}{0} & \parbox[t]{3.516cm}{\num{0}} \\
\end{tabular}
...
\capfig{0.3\textwidth}{figures/Current/resistor.png}{\label{fig:current:resistor}A simple resistor of length, $L$, cross-sectional area, $A$, made from a material with resistivity, $\rho$. A potential difference, $\Delta V$, is applied across the resistor, leading to an electric field and current in the resistor.}
...
\end{align*}
\begin{example}{A hairdryer is rated as consuming $\SI{1500}{W}$ when connected to an outlet with a $\SI{120}{V}$ potential difference. What is the resistance of the hairdryer, and how much current goes through it when it is running?}
Since the power of the hairdryer and the potential difference across it are known, we can easily determine its resistance:
\begin{align*}
P&=\frac{(\Delta V)^2}{R}\\
...
\item What are some examples of superconducting materials, and at what temperature do they become superconducting?
