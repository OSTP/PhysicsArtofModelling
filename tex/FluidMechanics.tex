...
\end{align*}
This is the condition for the object to float, and we can see that whether an object floats depends on its density. Consider a box whose density is equal to the density of water. In this case, $\rho_0/\rho_W=1$, and so $V_W/V_0=1$. This means that, in order for the box to float, a volume of water that is equal to the volume of the box must be displaced. So, the entire ball must be submerged. 

Now consider a box whose density is half that of water. We find that $\rho_0/\rho_W=0.5$, so we must have $V_W/V_0=0.5$. In order for the box to float, only half of it needs to be submerged. Finally, what about a box whose density is 1.5 times the density of water? In this case, one and a half boxes worth of water would have to be displaced in order for the box to float. Even when the entire box is submerged, not enough volume has been displaced in order for it to float, so the box will sink. 
