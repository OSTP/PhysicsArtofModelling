...
\begin{definitions}
	\textbf{Angular frequency:} The rate at which an angle changes with respect to time. Angular frequency describes this change by equating a cycle to circular motion, where one full movement around the circle (\SI{2\pi}{rad}) is equivalent to the completion of a cycle. SI units: [\SI{}{rad/s}]. Common variable(s): $\omega$.
	\medskip
\end{definitions}
