...
\begin{definitions}
	...
	\medskip
	\item \textbf{Force:} An interaction which causes a change in acceleration. SI units: [\SI{}{N}]. Common variable(s): $\vec F$.
	\medskip
	\item \textbf{Spring constant:} A value which describes how tense a spring is. Despite the contradiction with its name, the spring constant is not always constant, and is often dependant on $x$. SI units: [\SI{}{Nm^{-1}}]. Common variable(s): $k$.
	\medskip
	\item \textbf{Gravitational acceleration:} The acceleration (not force) due to gravity. SI units: [\SI{}{ms^{-2}}]. Common variable(s): $\vec g$.
	\medskip
	\item \textbf{Coefficient of friction:} A constant which states the amount of force produced by friction per newton of normal force. SI units: none. Common variable(s): $\mu$.
