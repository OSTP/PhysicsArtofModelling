...
\end{itemize}
%TODO Checkpoint question. A proton moves East, in Earth's magnetic field, which way is it deflected?
%TODO Checkpoint question. An electron moves West, in Earth's magnetic field, which way is it deflected?
...
%TODO obviously a much better figure with a pretty spiral and pretty magnetic field going through the plane. The trick might be to draw very few magnetic field lines.
...
%TODO A much bigger figure, try to make it stylistically similar to the galvanometer that Olivia made.
\capfig{0.3\textwidth}{figures/MagneticForce/dcmotor.png}{\label{fig:magneticforce:dcmotor}Illustration of a DC electric motor. Current circulate in the coil resulting in a torque from the magnetic field. Once the coil is aligned with the magnetic field, the direction of the current in the coil is inverted, so that the coil continues to feel a torque. The current is inverted using mechanical brushes that reverse the leads on the coil every half turn.}
...
\capfig{0.7\textwidth}{figures/MagneticForce/crt.png}{\label{fig:magneticforce:crt}Illustration of a cathode ray tube. A magnetic field is used to deflect a beam of electrons onto a screen. The beam is rapidly swept across the whole screen to create an image.}
...
\capfig{0.3\textwidth}{figures/MagneticForce/speaker.png}{\label{fig:magneticforce:speaker}Illustration of a loud speaker. As current moves through the coil, it is pushed up and down by the magnetic force exerted by a permanent magnet. The motion is transferred to a membrane that move the air and creates the sound wave.}
%TODO Obviously a much better figure!
...
%TODO Model a galvanometer: Given a torsion spring and spring constant, typical magnetic field, number of coils or a given area, what kind of current will create a given deflection? Look up some realistic values. Hopefully this gives a small current, as advertised in the Circuits chapter!

%TODO Calculate the magnetic field required to give a certain deflection (~10cm) in a CRT, for electrons accelerated with a typical potential in a CRT (500V ???)

%TODO Integrate the force to show that the torque on a circular loop of current is given by the magnetic moment (we never proved it, only stated that this was true and showed that it worked with rectangle loop).
