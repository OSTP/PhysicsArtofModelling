...
\begin{checkpoint}{Fill in the following table, giving the order of magnitude in meters of the sizes of different physical objects. Feel free to Google these!}
\begin{center}
\begin{tabular}{|c|c| }
\hline  
...
\begin{studentOpinion}{Emma}{\textbf{``Precision'', ``Accuracy'' and ``Uncertainty'' - what's the difference?}}
...
\textbf{Precision}
 is the repeatability of a measurement. If the values obtained are close to one another, they are precise. For example, say you were measuring the rebound height of a basketball, dropped from a fixed height. After performing the calculation multiple times, you find that your measurements are very close in value. Something that you could say about this data is: ``After repeating our measurement multiple times, the values that we obtained were very close together. Our measurements were very precise!''.

\textbf{Accuracy}
 measures the agreement between the actual value and the value that you measured. If the values are close to the accepted value, they are accurate. For example, say you performed calculations that estimated the distance that a slingshot will catapult a rock before it falls and stops. When we actually perform the experiment, our estimated value was the same as the actual value. You could describe your data as follows: ``Our estimated value was almost exactly the same as the actual value that we measured - it was really accurate!''.

\textbf{Uncertainty}
 is an estimate of the amount that a measurement will differ from an accepted value. In science, we aim to lower the uncertainty in our measurements. Let's say that you are measuring the number of rotations of a spinning top, spun with a fixed force. Your measurements are close together, but deviate from one another in a fixed range. This would be an example where you could calculate the uncertainty in your measurements. It would be sensible to say ``After multiple measurements, we've found that our values are similar. They deviate from one another within a fixed uncertainty.''
\end{studentOpinion}
...
\end{table}
\capfig{0.4\textwidth}{figures/ModelAndExperiment/ruler.png}{\label{fig:ModelAndExperiment:ruler}The length of the grey rectangle would be quoted as $L=\SI{2.8\pm0.5}{cm}$ using the rule of ``half the smallest division''.}
