...
E = K + U
\end{align*}
which will be a constant if there are no non-conservative forces doing work on the object. This means that if the potential energy of the object increases, then its kinetic energy must decrease by the same amount, and vice-versa. We can visualize this on an energy diagram, in which we can illustrate the potential energy as a function of position. 

Consider a block that can slide on a frictionless horizontal surface and that is attached to a spring, as in the top panel of Figure \ref{fig:potentialecons:springE}. If one pushes on the block so as to compress the spring by a distance $D$ and then releases it, the block will initially accelerate until it reaches the rest position of the spring. At that point, the spring will exert a force in the opposite direction and the block will continue in the same direciton and decelerate until it stops and turns around. It will then accelerate again towards the spring rest position, and then decelerate once the spring starts being compressed again, until the block stops and the motion repeats. 

Figure \ref{fig:potentialecons:springE} shows an energy diagram for an object attached to a spring, where the potential energy function is given by:
\begin{align*}
U(x)=\frac{1}{2}kx^2
\end{align*}
The $x$ axis is defined such that the origin is located where the spring is at rest. In the diagram, the object has a total energy of $E=\SI{25}{J}$, which is constant and shown by the horizontal line. Also illustrated are the potential energy function ($U(x)$ in blue), and the kinetic energy, ($K=E-U(x)$, in dotted black).
...
The energy diagram allows us to describe the motion of the object attached to the spring in terms of energy. Because the potential and kinetic energy must sum to $E$, we see that when the kinetic energy is zero, the potential energy is maximal. This corresponds to the spring being maximally compressed or extended, and is when the object will change direction of motion (the kinetic energy is zero, so the object is instantaneously at rest); we call these points ``turning points''. Conversely, when the potential energy is zero, the object has maximal kinetic energy, which corresponds to the object moving the fastest at the location where the spring is at rest. Furthermore, we can see that if the object has a larger total energy (which is equivalent to raising the horizontal line), then we see that the turning points will move further outwards, and the object will cover more distance as it oscillates back and forth.
