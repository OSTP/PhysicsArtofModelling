...
Although we introduced the kinetic energy as a way to calculate the integral for the net work, if one knows the value of the net work done on an object, then the Work-Energy Theorem can be used to calculate the change in speed of the object.
...
This is the same example as the previous one, but with kinetic friction. The forces on the object are:
\begin{enumerate}
\item $\vec F_g$, its weight, with magnitude $mg$.
\item $\vec N$, the normal force exerted by the ground.
\item $\vec F(x)$, the force from the spring, with magnitude $kx$. 
...
\end{align*}
Applying the work energy theorem, and noting that the object started at rest ($v_i=0$), the final speed $v_f$ is found to be:
\begin{align*}
