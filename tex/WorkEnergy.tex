...
\end{align}
where $\theta$ is the angle between the vectors when these are placed tail to tail. The dimension of work, force times displacement, is also called ``energy''. The S.I. unit for energy is the Joule (abbreviated $\si{J}$) which is equivalent to $\si{Nm}$ or $\si{kg m^2/s^2}$ in base units.
...
\begin{example}{\capfig{0.5\textwidth}{figures/WorkEnergy/workgravity.png}{\label{fig:workenergy:workgravity}Two possible paths to move a box from the floor onto a table. }
...
\begin{align}
\Aboxed{W = \int_A^B \vec F(\vec r) \cdot d\vec r}
\end{align}
...
\begin{example}{\capfig{0.4\textwidth}{figures/WorkEnergy/workparabola.png}{\label{fig:workenergy:workparabola}A parabolic path between $A$ and $B$. }
...
\capfig{0.2\textwidth}{figures/WorkEnergy/workparabola_dr.png}{\label{fig:workenergy:workparabola_dr}The infinitesimal displacement vector, $d\vec r$. }
...
\begin{example}{\capfig{0.3\textwidth}{figures/WorkEnergy/workincline.png}{\label{fig:workenergy:workincline} A crate being pushed up an incline.}
...
TODO: Recycle above example into the question library for Newton's Laws and ask for the magnitude of the force for the crate to move at constant speed. Can also ask to compare the magnitude of the normal force between the case when its constant speed and some given value of the acceleration.

\section{Kinetic energy and the work energy theorem}
At this point, you should be comfortable calculating the net work done on an object upon which several forces are exerted. In this section, we derive a new quantity, kinetic energy, which allows us to connect the work done on an object with its change in speed. As we saw in the previous section, the net work done on an object is connected to the objects acceleration; if the net force on the object is zero, then the net work done and acceleration are also zero. The following derivation also appears to ``come out of thin air'', but again, remember that theorists have tried all sorts of mathematical tricks to reformulate Newton's Theory, and this is the one that worked. 

Consider the most general case of an object of mass $m$ acted upon by a net force, $\vec F^{net}(\vec r)$, which can vary in magnitude and direction. We wish to calculate the  net work done on the object as it moves along an arbitrary path between two points, $A$ and $B$ in space, as show in Figure \ref{fig:workenergy:kepath}. The instantaneous acceleration of the object, $\vec a$, is shown along with an ``element of the path'', $d\vec r$. 
...
\begin{align*}
\int_A^B F^{net}(\vec r) \cdot d\vec r =K_B - K_A
\end{align*}
That is, the function $K$ is akin to an anti-derivative in the sense that it can be evaluated only at the end points of the path to determine the integral.
...
\therefore \int_A^B F^{net}(\vec r) \cdot d\vec r &= \int_A^B m\vec a\cdot d\vec r =m\int_A^B \vec a\cdot d\vec r
\end{align*}
where we assumed that the mass of the object does not change along the path and can thus be factored out of the integral. Consider the scalar product of the acceleration, $\vec a$, and the path element, $d\vec r=dx\hat x  +dy\hat y + dz\hat z$, written in terms of the velocity vector:
\begin{align*}
\vec a & = \frac{d\vec v}{dt}\\
\therefore \vec a\cdot d\vec r &= \frac{d\vec v}{dt}\cdot d\vec r\\
...
\begin{align*}
\vec a\cdot d\vec r&= \frac{dv_x}{dt}dx+\frac{dv_y}{dt}dy+\frac{dv_z}{dt}dz\\
&=\frac{dx}{dt}dv_x + \frac{dy}{dt}dv_y+\frac{dz}{dt}dv_z
\end{align*}
We can thus write the integral for the net work done as:
\begin{align*}
W^{net} &= \int_A^B F^{net}(\vec r) \cdot d\vec r =m \int_A^B (v_xdv_x + v_ydv_y + v_zdv_z)\\
...
\end{align*}
where we recognized that the magnitude (squared) of the velocity is given by $v_A^2 = v_{Ax}^2+v_{Ay}^2+v_{Az}^2$. We have thus arrived out our desired result, namely, we have found a function of speed, $K(v)$, that when evaluated at the endpoints of the path allows the net work done on the object to be calculated:
\begin{align}
...
Most importantly, the Work-Energy theorem introduces the concept of ``energy''. As we will see in later chapters, there are other forms of energy in addition to work and kinetic energy. The Work-Energy Theorem is the starting point for the idea that one can convert one form of energy into another. The Work-Energy Theorem tells us how a force, by doing work, can provide (or remove) kinetic energy to (or from) an object. 
...
\begin{example}{\capfig{0.4\textwidth}{figures/WorkEnergy/spring.png}{\label{fig:workenergy:spring2}A block is pressed against a horizontal spring so as to compress the spring by a distance $D$ relative to its rest length.} A block is pressed against the free end of a horizontal spring with spring constant, $k$, so as to compress the spring by a distance $D$ relative to its rest length, as shown in Figure \ref{fig:workenergy:spring2}. The other end of the spring is fixed to a wall. If the block is released from rest and there is no friction between the block and the horizontal surface, what is the speed of the block when it leaves the spring?}
...
\begin{example}{\capfig{0.4\textwidth}{figures/WorkEnergy/spring.png}{\label{fig:workenergy:spring3}A block is pressed against a horizontal spring so as to compress the spring by a distance $D$ relative to its rest length.} A block is pressed against the free end of a horizontal spring with spring constant, $k$, so as to compress the spring by a distance $D$ relative to its rest length, as shown in Figure \ref{fig:workenergy:spring3}. The other end of the spring is fixed to a wall. If the block is released from rest and the coefficient of kinetic friction between the block and the horizontal surface is $\mu_k$, what is the speed of the block when it leaves the spring?}
...
\begin{example}{You are pushing a crate along a horizontal surface at constant speed, $v$. You find that you need to exert a force of $\vec F$ on the crate in order to overcome the friction between the crate and the ground. How much power are you expending by pushing on the crate?}
...
\item Work is a scalar quantity that allows us to quantify how a force changes the speed of an object when that forces has been exerted over a certain distance.
\item If the net work done on an object is zero, that object does not accelerate.
\item The net work done on an object is equal to its change in kinetic energy.
\item Power is the rate at which work is being done, or energy converted.
}
\end{chapterSummary}
...
\begin{align*}
W^{net} = \int_A^B \vec F^{net}(\vec r) \cdot d\vec r
\end{align*}

One can define the kinetic energy, $K(v)$ of an object of mass $m$ that has speed $v$ as:
\begin{align*}
K(v) = \frac{1}{2} mv^2
\end{align*}
...
\begin{align*}
P = \frac{d}{dt} W =  \vec F \cdot \vec d = \vec F \cdot \frac{d}{dt}\vec d = \vec F \cdot \vec v
\end{align*}
