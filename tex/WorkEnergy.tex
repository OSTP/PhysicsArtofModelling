...
\item To construct the pyramids, the ancient Egyptians used simple machines, like levers, to accomplish tasks that would not be possible otherwise. Apply what we know about work to find out how levers enable people to lift incredibly heavy objects. 
...
\item Something about cars and power
\end{enumerate}
...
A child of mass $m$ sits on a swing of length $L$. You push the child with a horizontal force $\vec F$. You apply the force in such a way that the child moves at a constant speed (note that $\vec F$ will not have a constant magnitude).}
\item How much work do you do to move the child from an angle 0 to an angle $\theta$? 
\item Show that the work is equal to $mgh$, where $h$ is the change in height of the child. Hint: Set up your axes to take advantage of the fact that the child is moving along the $s$ axis. 
\end{problemParts}
...
\end{align*}
The net work is the sum of the work done by each of the three forces acting on the skier. These forces are gravity, friction, and the normal force, as shown in Figure \ref{fig:workenergy:skijumpprobfbd}.
...
\begin{enumerate}[label=\alph*)]
\item We want to find the work done by the applied force $F$. The magnitude of the force will change, so we first need to find an expression for the magnitude of the force that depends on $\theta$.
\end{enumerate}
\end{solution}
