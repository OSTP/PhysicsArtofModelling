...
\begin{opening}
Julie is holding a heavy book. The book does not move, even though it is difficult for her to keep it from falling to the ground. Do Julie's arms do work on the book? How about if Julie starts walking to class while holding the book? 
...
\subsection{Problems}
\begin{problemParts}{A ski jump can be modelled by a ramp, inclined at an angle of $\theta$ above the horizontal so that it has a height $h$. A skier of mass $m$ is moving at a speed $v_0$ when they start going up the ramp. When the skiier lands the jump, they are moving at a speed $v_f$. \label{prob:workenergy:q1}}
\item What is the speed of the skier the instant they leave the ski jump?
\item What is the coefficient of friction between the skier and the snow?
\end{problemParts}

\newpage
\subsection{Solutions}
\begin{solution}{\ref{prob:workenergy:q1}}
{
the solution
}
\end{solution}
