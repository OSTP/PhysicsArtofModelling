...
We can determine the speed at the top of the ramp, $v_t$, using the Work-Energy Theorem:
\begin{align*}
...
\end{align*}
where $d\vec l$ is an infinitesimal displacement, as shown in Figure \ref{fig:workenergy:skijumpprobdisplacement}. 
\capfig{0.7\textwidth}{figures/WorkEnergy/skijumpprobdisplacement.png}{\label{fig:workenergy:skijumpprobdisplacement}Infinitesimal displacement along the trajectory of the jump.}
...
\end{align*}
The work done by gravity is thus:
\begin{align*}
W = \int \vec F_g \cdot d\vec l = \int_h^0 -mgdy = [-mgy]_h^0 = mgh
\end{align*}
and is positive, which makes sense, since gravity is generally in the same direction as the net displacement (downwards). We did not need to take into account the specific shape of the trajectory, because the force was constant in magnitude and direction (see Example \ref{ex:workenergy:workparabola}).
...
\item We can again use the Work-Energy Theorem to determine the work done by friction as the skier slides up the ramp. We know that the speed of the skier at the bottom of the ramp is $v_i=\SI{15}{m/s}$, and we just found that the speed of the skier at the top of the ramp is $v_t=\SI{6.8}{m/s}$. The net work done on the skier going up the ramp is equal to:
\begin{align*}
W^{net}&=\frac{1}{2}mv_t^2-\frac{1}{2}mv_i^2\\
&=\frac{1}{2}m(v_t^2-v_i^2) = \frac{1}{2}(\SI{80}{kg})((\SI{6.8}{m/s})^2-(\SI{15}{m/s})^2)=\SI{-7150.4}{J}
\end{align*}
The net work done is the sum of the work done by each of the forces acting on the skier. The forces on the skier are gravity, the force of friction, and the normal force. The normal force does no work, since it is always perpendicular to the displacement. The net work is thus the sum of the work from gravity, $W_g$, and the work done by friction, $W_f$. 
\begin{align*}
W^{net}=W_g+W_f
\end{align*}
The work done by gravity is:
\begin{align*}
W_g = \vec F_g \cdot \vec d = (-mg\hat y) \cdot  (d_x\hat x + h \hat y) = -mgy
\end{align*}
where $\vec d$ is the displacement vector up the ramp (unknown horizontal distance, $d_x$, and vertical distance, $h$).

%%%%%%%%%%%%%%Resume here - better to do with variables to highlight the point made in the discussion....
