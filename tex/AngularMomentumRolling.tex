...
In this section, we develop the tools required to model the motion of an object rolling along a surface, such as that of a bicycle wheel. Consider the motion of a wheel of radius $R$ rotating with angular velocity, $\vec\omega$, about an axis perpendicular to the wheel and through its centre of mass, as observed in the centre of mass frame. This is illustrated in Figure \ref{fig:angularmomentumrolling:wheelcm}.
...
\end{align*}
where $\vec r$ is a vector (of magnitude $R$) from the centre of mass to the corresponding point on the edge of the wheel (shown in Figure \ref{fig:angularmomentumrolling:wheelcm} for one of the point). The vector $\vec r$ is always perpendicular to $\vec \omega$, so that the speed all points on the edge, as measured in the frame of reference of the centre of mass, is the same for each point:
\begin{align}
...
\end{align*}
That is, in the frame reference of the ground, each point will have a velocity obtained by (vectorially) adding its velocity relative to the centre of mass, $\vec v_{rot}$, and the velocity of the centre of mass relative to the ground, $\vec v_{CM}$. This is illustrated in Figure \ref{fig:angularmomentumrolling:wheelground} for one specific point, shown in red. The red vector corresponds to the velocity of the red point as the wheel rotates, and is obtained by vectorially adding the velocity of the centre of mass, $\vec v_{CM}$ and the velocity $\vec v_{rot}$ relative to the centre of mass (shown as the dashed vector, tangent to the edge of the wheel). 

Consider, specifically, the instant in time when the red point is at the bottom of the wheel, where the wheel makes contact with the ground. If the wheel is not slipping with respect to the ground, then the point is, at that instant, at rest relative to the ground. We call this type of motion ``rolling without slipping'', which is defined to be rolling motion when the object does not slip relative to the ground; in other words, the point on the rotating object that is in contact with the ground is instantaneously at rest relative to the ground. This is the scenario illustrated in Figure \ref{fig:angularmomentumrolling:wheelground}.

For the point in contact with the ground, the vectors $\vec v_{rot}$ and $\vec v_{CM}$ are anti-parallel and must sum to zero, for rolling without slipping. Writing out the horizontal component of the velocity of the point on the ground (choosing the positive direction to be in the direction of the velocity of the centre of mass):
\begin{align*}
v &= -v_{rot} + v_{CM} = 0\\
\therefore v_{rot} &= v_{CM}
\end{align*}
and we find that, for rolling without slipping, the magnitude of the velocity from rotation about the centre of mass has to equal to the speed of the centre of mass. The magnitude of the velocity from rotation about the centre of mass is given in terms of the angular velocity of the wheel about the centre of mass by Equation \ref{eq:angularmomentumrolling:vrot}. For rolling without slipping, we thus have the following relationship between angular velocity and the speed of the centre of mass:
\begin{align}
\Aboxed{\omega R &= v_{CM}}\quad \text{(rolling without slipping)}
\end{align}
It makes sense for the angular velocity to be related to the speed of the centre of mass. The faster the wheel rotates, the faster the centre of mass will move. If the wheel is slipping with respect to the ground, then the point of contact is no longer stationary relative to the ground, and there is no relation between the angular velocity and the speed of the centre of mass. 

The magnitude of the linear acceleration of the centre of mass, $a_{CM}$, is similarly related to the magnitude of the angular acceleration of the wheel, $\alpha$, about the centre of mass:
\begin{align*}
a_{CM} &= \frac{dv_{CM}}{dt} = \frac{d}{dt}\omega R = R \frac{d\omega}{dt}\\
\therefore a_{CM} &= R\alpha
\end{align*}

TODO: Checkpoint MC question: What is the speed of the top of the wheel (answer: 2 vcm)

When an object is rolling without slipping, we can model its motion as the superposition of rotation about the centre of mass and translational motion of the centre of mass, as we just did. However, because the point of contact between the rolling object and the ground is stationary, we can also model the motion as if the object were instantaneously rotating with angular velocity, $\vec \omega$, about an axis through the point of contact. That is, we can model the motion as rotation only, with no translation, if we choose an axis of rotation through the point of contact between the ground and the wheel.

We call the axis through the point of contact the ``instantaneous axis of rotation'', since, instantaneously, it appears if the whole wheel is rotating about that point. This is illustrated in Figure \ref{fig:angularmomentumrolling:wheelinstant}, which shows, in red, the velocity vector for each point on the edge of the wheel, relative to the instantaneous axis of rotation. Because the axis of rotation is fixed to the ground, the velocity of each point about that axis of rotation corresponds to the same velocity relative to the ground that is depicted in Figure \ref{fig:angularmomentumrolling:wheelground}.
...
In particular, the angular velocity, $\vec \omega$ about the instantaneous axis of rotation is the same as if we model the motion as translation plus rotation about the centre of mass. Indeed, relative to the instantaneous axis of rotation, the centre of mass must still have a velocity $\vec v_{CM}$, which is given by:
\begin{align*}
\vec v_{CM} &= \vec\omega \times \vec r_{CM}\\
\therefore v_{CM} &= \omega R
\end{align*}
where $\vec r_{CM}$ is the vector from the axis of rotation to the centre of mass. Similarly, the velocity of any point on the wheel, relative to the ground, is given by:
\begin{align*}
...
\begin{example}{\capfig{0.5\textwidth}{figures/AngularMomentumRolling/diskslope.png}{\label{fig:angularmomentumrolling:diskslope}A disk rolling without slipping down an incline.} A disk of mass $M$ and radius $R$ is placed at height $h$ on an incline that makes an angle $\theta$ with respect to the horizontal, as shown in Figure \ref{fig:angularmomentumrolling:diskslope}. If the disk starts at rest and rolls without slipping down the incline, what speed will the centre of mass have when the disk reaches the bottom of the incline? What is the angular acceleration of the disk, about an axis through its centre of mass, as it rolls down the slope?}
...
\omega = \frac{v_{CM}}{R}
\end{align*}
The moment of inertia of disk about its centre of mass is given by:
\begin{align*}
I_{CM}=\frac{1}{2}MR^2
\end{align*}
...
\begin{example}{\capfig{0.5\textwidth}{figures/AngularMomentumRolling/diskslope.png}{\label{fig:angularmomentumrolling:diskslope2}A disk rolling without slipping down an incline.} A disk of mass $M$ and radius $R$ is placed at height $h$ on an incline that makes an angle $\theta$ with respect to the horizontal, as shown in Figure \ref{fig:angularmomentumrolling:diskslope2}. What is the angular acceleration of the disk, about an axis through its centre of mass, as it rolls down the slope?}
...
a_{cm} = \alpha R
\end{align*}
The $x$ component of Newton's Second Law can thus be written:
\begin{align*}
Mg\sin\theta - f_s &= M \alpha R\\
\therefore f_s &= Mg\sin\theta - M\alpha R
\end{align*}
Which allows us to substitute out the force of friction from our previous model for the angular acceleration:
\begin{align*}
\alpha &= \frac{2f_s}{MR}\\
...
Instead of modelling the motion of the disk about the centre of mass, we can also model it about the instantaneous axis of rotation, where the disk makes contact with the incline. The angular acceleration about that point will be the same as the angular acceleration about the centre of mass. About the instantaneous axis of rotation, only the force of gravity can exert a torque, since the normal force and the force of friction both have a lever arm of zero. The torque from the force of gravity, about the instantaneous axis of rotation is:
\begin{align*}
\vec \tau_g = -F_gR\sin\theta \hat z = -MgR\sin\theta \hat z 
\end{align*}
This (net) torque is equal to the moment of inertia of the disk about the instantaneous axis of rotation, $I$, multiplied by its angular acceleration:
\begin{align*}
\tau ^{ext} = \tau_g &= I\alpha\\
...
If an object is rotating with angular speed $\omega$ about an axis that is fixed in space, the rotational kinetic energy of that object is given by:
\begin{align*}
K_{rot} = \frac{1}{2}I\omega^2
\end{align*}
where $I$ is the moment of inertia of that object about the axis of rotation.

The work done by the net torque exerted on an object about a fixed axis or rotation is equal to object's change in rotational kinetic energy:
\begin{align*}
...
If an object of mass $M$ is rotating about an axis through its centre of mass, and the centre of mass of is moving with speed $v_{CM}$, then the total kinetic energy of the object is given by:
\begin{align*}
...
Many of the relations that were determined between linear quantities have an equivalent relation between the corresponding quantities, which we summarize in the table below:
