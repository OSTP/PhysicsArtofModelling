...
For this problem we can determine whether the force is attractive or repulsive by finding the direction of $\vec{F_2}$, the force on wire 2, and the direction of $\vec{F_1}$, the force on wire 1. Figure \ref{fig:magneticsource:2wires_steps} shows how to find the directions of $\vec{F_1}$ and $\vec{F_2}$ step by step using the right hand rules. The process is as follows: 
\begin{enumerate}[label=(\alph*)]
\item Use the axial right hand rule to find the magnetic field at wire 2 due to the current in wire 1.
...
\item Use the axial right hand rule to find the magnetic field at wire 1 due to the current in wire 2.
...
Both $\vec{F_1}$ and $\vec{F_2}$ point towards the centre, and the force is attractive. See what happens when you use your left hand!
\end{studentOpinion}
...
\begin{studentOpinion}{Olivia}
Let's compare Ampere's law to Gauss's law. In the chapter on Gauss's law, I showed that when we are dealing with a high degree of symmetry, we are able to write:
\begin{align*}
\oint \vec{E}\cdot d\vec{A}=\frac{Q^{enc}}{\epsilon_0}\rightarrow EA=\frac{Q^{enc}}{\epsilon_0}
\end{align*}
where $E$ is the magnitude of the electric field, $A$ is the surface area of the Gaussian surface, and $Q^{enc}/\epsilon_0$ gives us the flux. So, to find the electric field at any point on the surface, we use
\begin{align*}
...
\end{align*}
where $B$ is the magnitude of the magnetic field, $l$ is the length of the Amperian loop, and $\mu_0 I^{enc}$ gives us the circulation. To find the magnetic field, we use
\begin{align*}
B=\frac{\mu_0 I^{enc}}{l},
\end{align*}
