...
%This heat map could go from black to white, which would blend well with the textbook, but the colourcheme is not exactly the perfect example of a heatmap, and it seems like we are assuming that stuents are not familiar with heatmaps (otherwise, todo done)
...
%Do we want this to have a heatmap as well, or no? Heatmap can be easily taken out (otherwise, todo done)
...
%The rolled up capacitor needs to actually be drawn!
%This rolled up capacitor looks pretty good. It is a bit more of a sprial thana  cylinder, but that can be fixed if need be otherwise, todo done
...
{
\item Try to release a static discharge from your finger to some metal object. Measure the distance between your finger and the metal object at the time of the discharge. Knowing the breakdown voltage of air, what was the electric field between your finger and the metal object at the time of the discharge?
}
\end{chapteractivity}
...
Consider a long cylinder of radius $R = N$ and charge density $\rho = N$. If the electric potential at the surface of the sphere is $V_S = \SI{100}{V}$, then what is the electric potential inside and outside of the cylinder as a function of $r$?}

\end{problem}

\begin{problem}{soln:template:ballistic}{\label{prob:template:ballistic} 
		Consider two nested conducting cylinders of length $L$ which are seperated by the distance between the points $a$ and $b$, which are both measured radially from the centre of the nested cylinders, as shown in Figure \ref{fig:potential:2pos1neg}. If the linear charge density of both cylinders is $\lambda$, what is the capacitance between points $a$ and $b$?}
...
\begin{solution}{prob:template:ballistic}\label{soln:template:ballistic}
	...
	To calculate the electric potential inside and outside of the sphere, we must first calculate the electric field. We will do this using Gauss' Law. We will define our enclosed charge using a cylinder of radius $r$ which is less than $R$ and length $L$:
