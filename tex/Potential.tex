...
\section{Capacitors}
Capacitors are common electronic devices that are used to store electric charge. Capacitors are often used when one needs a large amount of charge at once (e.g. to create a bright flash for a camera), or when one needs to collect charges in order to measure a quantity of charge. A capacitor is usually constructed with two conducting plates separated by either air or an insulating material.
