...
Because the momentum of a particle is defined using the velocity of the particle, its value depends on the reference frame that we use to measure the velocities. In some cases, it is useful to apply momentum conservation in a frame of reference where the total momentum of the system is zero. For example, consider two particles of mass $m_1$ and $m_2$, moving towards each other with velocities $\vec v_1$ and $\vec v_2$, respectively, as measured in a frame of reference $S$, as illustrated in Figure \ref{fig:momentumandcm:2particles}.
...
In the frame of reference $S$, the total momentum of the two particles can be written:
\begin{align*}
...
\end{align*}
The total momentum in the frame of reference $S'$ is then given by:
\begin{align*}
...
\end{align*}
This ``special'' frame of reference, in which the total momentum of the system is zero, is called the ``centre of mass frame of reference''. The velocity for centre of mass frame of reference can easily be obtained if there are $N$ particles involved instead of two:
\begin{align*}
\therefore \vec v_{CM} = \frac{m_1\vec v_1 + m_2\vec v_2 + m_3 \vec v_3 + \dots }{m_1+m_2+m_3+\dots}=\frac{\sum_i^N m_i\vec v_i}{\sum_i^N m_i}
\end{align*}


TODO: Example with closest approach of the two masses with spring elastic collision
