...
\item Choose a gaussian surface that goes through the point for which you want to know the electric field. Ideally, the surface is such that the electric field is constant in magnitude and always makes the same angle with the surface (ideally, the field is parallel or perpendicular to the surface). This will make the flux easy to calculate.
...
We start by making a diagram of the charge distribution so that we can use symmetry arguments to determine the direction of the electric field vector. At any point in space, the electric field vector must be radial (point to/from the centre of the wire) and in the plane perpendicular to the wire. If there were not the case, one would be able to look at the electric field to determine a preferred direction (either around the wire, if the field were not radial, or upwards/downwards, if the field were not perpendicular to the wire). 
...
\end{align*}
where you should note that the closed integral ($\oint$) was separated into three normal integrals ($\int$) corresponding to the three ``open'' surfaces that make up the closed surface. Again, remember that the flux is proportional to the net number of field lines exiting/entering the closed surface, so it make sense to count those lines over the three surfaces and add them together to get the total number for the closed surface.
...
\end{align*}
where we recognized that the side surface can be unfolded into a rectangle of height, $L$, and width, $2\pi R$, corresponding to the circumference of the cylinder. 

Next, we determine the charge inside the volume enclosed by the surface. Since the cylinder encloses a length, $L$, of wire, then the enclosed charge is given by:
\begin{align*}
Q^{enc}=\lambda L
\end{align*}
...
\end{example}
%TODO Checkpoint question: Why can't Gauss' Law be applied to a finite wire? A) It can, B) Because the symetry argument does not hold (correct), C) because the net charge needs to be infinite? I can't think of good wrong answers....
...
Figure \ref{fig:gauss:plane} shows a portion of the infinite plane. The electric field vector must be perpendicular to the plane or a preferred direction could otherwise be inferred from the direction of the electric field. We can also argue that above any point on the plane, there is an equal amount of charge in all directions along the plane, so all but the vertical components of the electric fields from those charges will cancel. The electric field will also need to point away from (towards) the plane, if the charge is positive (negative), or one would be otherwise be able to use the electric field to define ``up'' and ``down'', which breaks the symmetry of the plane. 
...
To determine the electric field, we choose a gaussian surface that is a box (as in Example \ref{ex:gaus:infiniteplate}), but require the lower end of the box to go through the plane, as illustrated in Figure \ref{ex:gaus:infiniteplate}. With this choice of gaussian surface, only the top surface (area $a^2$) will have flux through it, since the \textbf{electric field inside a conductor must be zero}\footnote{Since charges can freely move in a conductor, they will move until there is no reason to move. Eventually, the charges accumulate in such a way that the net field in the conductor is zero. For a plane, this means that half of the charges will move to each side.}. The total flux is given by:
\begin{align*}
...
\end{align*}
where we used the fact that only half of the charges are inside the surface of the plane enclosed by our gaussian surface, so that the charge per unit area is half ($\frac{\sigma}{2}$) of that for the entire plane. Applying Gauss' Law, we find that the electric field is given by:
\begin{align*}
\oint \vec E\cdot d\vec A&=\frac{Q^{enc}}{\epsilon_0} \\
...
%TODO: The next figure can be improved (e.g. the + and - signs should be more uniform...)
Consider, now, a neutral spherical conducting shell, as shown in the left panel of Figure \ref{fig:gauss:sphereshell}. When a charge, $+Q$, is placed at the centre of the shell, charges inside the shell will move until the field in the shell is identically zero. The negative charges will move towards the inner surface (as they are attracted to $+Q$) and positive charges will be repelled onto the outer surface, under the influence of the electric field created by $+Q$ (shown in the diagram as $\vec E_{Q}$). Eventually, the separation of charges will lead to an electric field (shown in the diagram as $\vec E_{\sigma}$) in the opposite direction. The charges will stop moving once the total electric field in the conductor is zero (when the two fields cancel exactly everywhere in the conductor).
...
Recall that flux is equivalent to counting the net number of field lines that exit a closed surface. Field lines must start on a positive charge and must end on a negative charge. If there are no net field lines coming out of the surface (or going in), then the surface cannot contain a ``source'' of field lines (a positive charge) or a ``sink'' for field lines to end (a negative charge). If there are as many field lines coming in as there are going out of a specific volume (defined by the gaussian surface), then there can be no net charge in that volume. If, on the other hand, there is a positive charge in that volume, the net number of field lines coming out of the volume will be proportional to the charge inside. 

The flux out of a closed surface is tightly connected to the vector calculus concept of ``divergence'', which describes whether field lines are diverging (spreading out or getting closer together). When a point charge is present, field lines will emanate radially from that point charge; in other words, they will diverge. We say that the electric field has non-zero divergence if there is a source of the electric field in that position of space. The key difference between the concept of divergence and that of ``flux out of a closed surface'', is that divergence is a local property of the field (it is true at a point), where the flux out of a surface must be calculated using a finite volume and cannot necessarily be used to describe the field at a particular point (in the examples of this chapter, we dealt with scenarios that allowed the electric field to have the same value over the whole gaussian surface, so we avoided this issue). The flux method is thus not as useful for describing how the field changes at specific positions, and usually limited to situations with a high degree of symmetry.

The divergence, $\nabla \cdot \vec E$, of a vector field at some position is defined as:
\begin{align*}
\nabla \cdot \vec E=\die{E}{x}+\die{E}{y}+\die{E}{z}
\end{align*}
and corresponds to the sum of these three partial derivatives evaluated at that position in space. Gauss' Theorem (also called the Divergence Theorem) states that:
\begin{align*}
...
\end{align*}
where $\rho$ is the charge per unit volume at a specific position in space. This is the version of Gauss' Law that is usually seen in advanced textbooks and in Maxwell's unified theory of electromagnetism. This version of Gauss's Law relates a local property of the field (its divergence) to a local property of charge at that position in space (the charge per unit volume at that position in space). If we integrate both sides of the equation over volume, we recover the original formulation of Gauss' Law: the left hand side, by the Divergence Theorem leads to flux when integrated over volume, whereas on the right hand side, the integral over volume of charge per unit volume, $\rho$, will give the total charge enclosed in that volume.
...
\Phi_E = \vec E \cdot \vec A = EA\cos\theta
\end{align*}
where $\vec A$ is a vector that is perpendicular to the surface with a magnitude equal to the area of that surface, and $\theta$ is the angle between $\vec A$ and $\vec E$. The flux of a field through a surface is proportional to the number of field lines that cross that surface. If the surface is parallel to the field ($\vec A$ and $\vec E$ are thus perpendicular), the flux will be zero (no field lines cross the surface, the scalar product is zero).

If $\vec E$ and $\vec A$ change over the surface ($\vec E$ and/or $\vec A$ change magnitude and/or direction along the surface), then we can subdivide the surface into little area elements over which the two vectors are constant. We define a vector $d\vec A$ to be perpendicular to the area element with an infinitesimal area, $dA$. The total flux is then obtained by summing the fluxes through each area element:
\begin{align*}
\Phi_E=\int \vec E \cdot d\vec A=\int EdA\cos\theta
\end{align*}
Note that the direction of the vector $d\vec A$ (or $\vec A$) is ambiguous, as one can choose either of two directions along a line that is perpendicular to a surface. Usually, one chooses the direction of $\vec A$ so that the flux is positive (i.e. $\vec A$ has a component parallel to $\vec E$). However, if the surface is ``closed'' (that is, it defines a volume), then we always choose the direction of $d\vec A$ so that it points outwards (since the surface encloses a volume, one can define is one is within or outside the volume). 
...
\oint \vec E \cdot d\vec A = \frac{Q^{enc}}{\epsilon_0}
\end{align*}
Electric field lines must begin and end on a charge and the flux over a closed surface corresponds to counting the net number of field lines that exit (or enter) the surface. If there is no charge inside the volume, then the same number of field lines that enter the surface must exit it as well, since there is no place in the volume for field lines to start or end.

Gauss' Law is useful to determine the electric field. However, this can only be done analytically for charge distributions with a very high degree of symmetry. This is because the flux is not usually trivial unless:
\begin{enumerate}
\item \textbf{The electric field makes a constant angle with the surface}. When this is the case, the scalar product can be written in terms of the cosine of the angle between $\vec E$ and $d\vec A$, which can be taken out of the integral if it is constant:
\begin{align*}
\oint \vec E\cdot d\vec A=\oint E\cos\theta dA=\cos\theta\oint EdA
...
%TODO Electric field inside and outside a sphere with total charge Q that is distributed as rho = a r^2 - or something like that, but this may already exist in the Question library. BAsically, a sphere where you have to integrate to get the charge enclosed.

%TODO Surface charge density at the inside and outside of a conducting shell with charge Q at the centre (as in the narrative above). From Gauss' Law, you know that the charge on the inner surace is -Q, and you know that that it has area 4 pi r_in^2. Same for outer surface, highlight that charge density will be different in the two cases (unlike the infinite plane). <-- Maybe too close to the example done in charge on a conductor?

%TODO An example of using the div version of gauss' law (1D line of charge maybe, so rho(x,y,z) -> lambda(x) and then it becomes an ODE? 
